...
var saveamuda; var maxvalue; 
var ezerboard; var saveboard;
var howmanyseedaddtostore = 0;

var anotherturnbool = false;
function TurnComputer() {
    if (!turn) {
        var k; maxvalue = temppits * Number(seednum) * -2;
        for (k = 1; k < temppits+1; k++) {
            ezerboard = CopyArray(mancala);
            if (ezerboard[1][k] != 0) {
                CheckPlayer2(k);
                if (anotherturnbool === true)
                    anotherturn[k - 1] = -1;
                anotherturnbool = false;
                var value = ValueBoard();
                if (value > maxvalue) {
                    maxvalue = value;
                    saveboard = ezerboard;
                    saveamuda = k;
                }
                howmanyseedaddtostore = 0;
            }
        }
        if (anotherturn[saveamuda-1] === -1) {
            turn = false;
            ...
            turn = true;
        mancala = saveboard;
        var savenumanimation = temppits + 2 + saveamuda-1;
        document.getElementById(savenumanimation).style.animation = "blink 1s 3";
        setTimeout("BuildBoard()", 3000);
        for (var d = 0; d < temppits; d++) //איפוס מערך של עוד תור
            anotherturn[d] = 0;
        maxvalue = 0;
...
function ValueBoard() {
    ...
    //        if (i === 1 && k != 0) {
    //            marblecomp = marblecomp + ezerboard[i][k];
    //            if (anotherturn[k] === -1)
    //                anotherturnval = anotherturnval + 20;
                
    //        }
    //        if (i === 0 && k != temppits) {
    //            marblep1 = marblep1 + ezerboard[i][k];
    //        }
    //        nikud = nikud + howmanyseedaddtostore * 200;
    //    }
    //}
    //nikud = nikud  + (ezerboard[1][0] - ezerboard[0][temppits]) + (marblecomp - marblep1) + anotherturnval;
    var last = true;
    var seedlost= 0;
    for (var i = 0; i < 2; i++) {
        for (var k = 0; k < temppits; k++) {
            if (i === 1 && k != 0) {
                if (anotherturn[k] === -1)
                    anotherturnval = anotherturnval + 2;

            }
            else if (ezerboard[1][1] != 1)
                last = false;
            else if(ezerboard[1][1] === 1 && k != 0 && k != 1) {
                if (ezerboard[i][k] != 0)
                    last = false;
            }
    ...
    if (last === true) {
        nikud = (ezerboard[1][0] - ezerboard[0][temppits]) + anotherturnval + seedlost;
        for (var k = 0; k < temppits - 1; k++) {
            seedlost = seedlost - ezerboard[0][k];
        }
    }
    else
        nikud = (ezerboard[1][0] - ezerboard[0][temppits]) + anotherturnval;
...
function CheckPlayer2(k) { // שחקן 2
    document.getElementById("turns").innerHTML = "It's your turn, " + p1name;
    var CurrentRow = 1;
    var CurrentCol = k;
    var tempseed = ezerboard[CurrentRow][CurrentCol];
    ezerboard[CurrentRow][CurrentCol] = 0;
    var numseedend = 0;
    while (numseedend != tempseed) {
        while (CurrentCol > 0) { //התקדמות לעבר הקופה
            CurrentCol--;
            ezerboard[CurrentRow][CurrentCol] += 1;
            numseedend++;
            if (numseedend == tempseed) {
                if (ezerboard[CurrentRow][CurrentCol] - 1 == 0 && ezerboard[CurrentRow - 1][CurrentCol-1] != 0 && CurrentCol != 0) {
                    ezerboard[1][0] += ezerboard[CurrentRow - 1][CurrentCol-1] + 1;
                    ezerboard[CurrentRow][CurrentCol] = 0;
                    ezerboard[CurrentRow - 1][CurrentCol-1] = 0;
                    // בדיקה אם הייתה העברה של גולה למקום ריק. אם כן תתבצע אכילה של הגולות של השחקן השני שנמצאות בצד שלו 'באותה' העמודה
                }
                break;
            }
        ...
        if (CurrentCol == 0) { // הגעה לקופה
            if (numseedend == tempseed) { // אם הגולה האחרונה נפלה בקופה השחקן מקבל תור נוסף
                anotherturnbool = true;
                howmanyseedaddtostore++;
                break;
            }
            CurrentRow--;
